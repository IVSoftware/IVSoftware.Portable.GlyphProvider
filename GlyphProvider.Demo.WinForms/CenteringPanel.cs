...
using IView = System.Windows.Forms.Control;
...
namespace IVSGlyphProvider.Demo.WinForms
{
    public class CenteringPanel : Panel, IConfigurableLayoutStack
    {
        ...
        public class GridAdaptor : TableLayoutPanel
        {
            public GridAdaptor()
            {
            }
            ...
            public TableLayoutRowStyleCollection RowDefinitions => RowStyles;
            public TableLayoutColumnStyleCollection ColumnDefinitions => ColumnStyles;

            internal void Add(IView control, int col, int v)
            {
                throw new NotImplementedException();
            }
        }
        public GridAdaptor Grid
        {
            get
            {
                if (_grid is null)
                {
                    _grid = new GridAdaptor();
                }
                return _grid;
            }
        }
        GridAdaptor? _grid = null;
            ...
            bool overwriteDefaults = false) where T : struct, Enum
        {
            ...
            void localStageComponents()
            {
                ...
                for (int col = 0; col < elements.Length; col++)
                {
                    ...
                    if (enumIdButton.WithCompatibility() is { } eicpa)
                    {
                        ...
                        eicpa.BackgroundColor = uniformBackgroundColor;
                        eicpa.TextColor = uniformTextColor;
                        eicpa.FontSize = UniformFontSize;
                        eicpa.Padding = 0;
            ...
            void localConfigVertical()
            {
                ...
                for (int row = 0; row < components.Count; row++)
                {
                    ...
                    if (component.WithCompatibility() is { } eicpa)
                    {
                        if (displayFormatOptions.HasFlag(DisplayFormatOptions.ShowMember))
                        {
                            eicpa.Text = eicpa.EnumId?.ToString() ?? "?";
                        }
                    }
                }
            }
...
#endif
                // Migration path
                if (idButton.WithCompatibility() is { } eicpa)
                {
                    switch (displayFormatOptions)
                    {
                        case DisplayFormatOptions.ShowSquareIcon:
        ...
        PortableThickness _uniformThickness = new(2);
        ...
        public float UniformFontSize
        {
            ...
            set
            {
                if (!Equals(UniformFontSizeNE, value))
                {
        ...
        public float UniformFontSizeNE { get; set; } = IVSoftware.Portable.Demo.Constants.DEFAULT_FONT_SIZE;

        IGridLayoutInterop IConfigurableLayoutStack.Grid => throw new NotImplementedException();
        ...
        public IBindableDlgtInterop Add(object o)
        {
            var eicpa = o.As<IPlatformComponentInterop>();
            if (eicpa.Handle is IView view)
            {
                switch (Orientation)
                {
                    case LayoutOrientation.Horizontal:
                        Grid.ColumnDefinitions.Add(new());
                        Grid.Add(view, Grid.ColumnDefinitions.Count-1, Grid.RowDefinitions.Count-1);
                        break;
                    case LayoutOrientation.Vertical:
                        Grid.RowDefinitions.Add(new());
                        Grid.Add(view, Grid.ColumnDefinitions.Count-1, Grid.RowDefinitions.Count-1);
        ...
        IPlatformComponentInterop IConfigurableLayoutStack.Add(object o)
        {
            throw new NotImplementedException();
        }
    }
